...
问答不是把问题直接丢给模型。浏览器先在本机检索已收录法条，再把命中的条文交给**你自己配置**的大模型整理。本站没有问答后端，提问和 Key 都不会经过我们的服务器。
...
<div class="jl-arch">
  ...
  <p class="jl-arch__note">因此答案质量取决于两件事：本机有没有检索到对的法条，以及你选用的模型是否遵守「只依据给定条文」的约束。重大事项仍应咨询执业律师。</p>
</div>
