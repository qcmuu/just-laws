"""Hybrid retrieval: dense (vector search) + sparse (BM25/jieba).

The dense side queries the configured vector backend (``VECTOR_BACKEND``):
pgvector (default) via ``db.search`` or the local Chroma collection — both
built by ``ingest.py``. The sparse side builds a BM25 index over
``data/chunks.jsonl`` tokenized with jieba. The two ranked lists are merged
with Reciprocal Rank Fusion (default) or a min-max normalised weighted sum,
both configured via environment variables (see ``config.py``).
...
def dense_search(question, n, category=None):
    ...
    Dispatches on ``config.VECTOR_BACKEND`` (pgvector default, or chroma).
    """
    if config.VECTOR_BACKEND == "chroma":
        return _dense_search_chroma(question, n, category=category)
    return _dense_search_pgvector(question, n, category=category)


def _dense_search_pgvector(question, n, category=None):
    import db

    qvec = embeddings.embed(question, is_query=True)[0]
    return db.search(_pg_connection(), qvec, top_k=n, category=category)


def _dense_search_chroma(question, n, category=None):
    qvec = embeddings.embed(question, is_query=True)[0]
